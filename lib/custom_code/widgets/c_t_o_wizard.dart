...
class CTOWizard extends StatefulWidget {
  const CTOWizard({
    Key? key,
    this.width,
    this.height,
  ...
  @override
  _CTOWizardState createState() => _CTOWizardState();
}

class _CTOWizardState extends State<CTOWizard> {
  // --- STATE ---
  ...
  Future<void> _fetchMaintenanceData() async {
    ...
    try {
      final url = Uri.parse(
          'https://fleet.equipmetry.kz/api/v1/workspace/service/maintenances/${widget.maintenanceId}');
  ...
  Future<void> _fetchRelatedParts() async {
    ...
    try {
      final url = Uri.parse(
          'https://fleet.equipmetry.kz/api/v1/workspace/part-nomenclature/spare-parts?workspace_id=${widget.workspaceId}&equipment_id=$_equipmentId');
      final response = await http.get(url, headers: {
        'Authorization': widget.token,
        'Workspace': widget.workspaceId
      });
  ...
  void _selectPart(dynamic originalPart, int qty, {String? uniqueKey}) {
    ...
    final key = uniqueKey ??
        originalPart['id'] ??
        originalPart['product_article'] ??
        originalPart['part_number'];
  ...
  Future<void> _finish() async {
    // 1. Сохраняем Задачи в AppState.CTOWorks (List<WorksStruct>)
    final selectedTasksList =
        _tasks.where((t) => _selectedTaskIds.contains(t['task_id'])).toList();
    try {
      List<WorksStruct> worksStructs = [];

      for (var task in selectedTasksList) {
        worksStructs.add(createWorksStruct(
          title: task['title'] ?? '',
          quantity: 1, // По умолчанию 1 работа
          // defectId: null, // Если есть в API, добавьте: task['defect_id']
          price: 0, // По умолчанию 0
          serviceStationComment: task['description'] ?? '',
          maintenanceId: widget.maintenanceId,
          workspaceId: widget.workspaceId,
        ));
      }

      FFAppState().CTOWorks = worksStructs;
    } catch (e) {
      print("Err saving works: $e");
    }

    // 2. Сохраняем Запчасти в AppState.CTOItems (List<InventoryItemsStruct>)
    try {
      List<InventoryItemsStruct> itemsStructs = [];

      _selectedPartsMap.values.forEach((partMap) {
        // Маппинг полей
        String titleVal = partMap['title'] ?? partMap['name'] ?? '';
        String articleVal =
            partMap['product_article'] ?? partMap['part_number'] ?? '';
        String uomVal = partMap['unit_of_measurement'] ?? '';

        // Кол-во (выбранное пользователем)
        int qtyVal = partMap['quantity'] is int
            ? partMap['quantity']
            : (int.tryParse(partMap['quantity'].toString()) ?? 0);

        // Цена (если есть)
        double priceVal = 0.0;
        if (partMap['price'] != null) {
          priceVal = partMap['price'] is int
              ? (partMap['price'] as int).toDouble()
              : (partMap['price'] as double);
        }

        itemsStructs.add(createInventoryItemsStruct(
          title: titleVal,
          productArticle: articleVal,
          quantity: qtyVal,
          unitOfMeasurement: uomVal,
          price: priceVal,
          inStock: true, // Логика "в наличии", можно усложнить если надо
          serviceStationComment: '',
        ));
      });

      FFAppState().CTOInventoryItems = itemsStructs;
    } catch (e) {
      print("Err saving inventory items: $e");
    }
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Expanded(
            child:
                _currentStep == 1 ? _buildStep1_Tasks() : _buildStep2_Parts(),
          ),
        ],
      ),
    );
      ...
      child: Row(
        ...
        children: [
          TextButton(
            ...
            child: Text(_currentStep == 1 ? "Отменить" : "Назад",
          ...
          TextButton(
              ...
              if (_currentStep == 1) {
                if (_selectedTaskIds.isEmpty) {
                  ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text("Выберите работы")));
                  return;
                }
                _fetchRelatedParts();
                setState(() {
            ...
            child: Text(_currentStep == 1 ? "Далее" : "Готово",
  ...
  Widget _buildCompactHeader() {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(
              "${equip['brand_id']['name']} ${equip['model_id']['name']} (${equip['license_plate_number']})",
              style: const TextStyle(
                  fontWeight: FontWeight.bold, color: Color(0xFF14181B))),
          Text("Пробег: ${_maintenanceData!['current_value']} км",
        ...
        Padding(
          ...
          child: Row(
            ...
            children: [
              ...
              TextButton(
                onPressed: () => setState(() => _selectedTaskIds =
                    _tasks.map((e) => e['task_id'].toString()).toSet()),
        ...
        child: Container(
          ...
          child: Text(title,
              ...
              style: TextStyle(
                  fontWeight: FontWeight.bold,
                  fontSize: 12,
                  color: active ? const Color(0xFF4B39EF) : Colors.grey)),
        ),
      ),
    );
  }
  ...
  Widget _buildPartsFromTasks() {
    ...
    if (activeTasks.isEmpty)
      return const Center(child: Text("Нет выбранных работ"));
                    ...
                    children: [
                      ...
                      if (inventoryItems.isNotEmpty)
                        Container(
                          ...
                          child: Text("${inventoryItems.length} ТМЦ",
                              style: TextStyle(
              ...
              if (isExpanded)
                if (inventoryItems.isEmpty)
                  const Padding(
                      ...
                      child: Text("Нет привязанных запчастей",
                          style: TextStyle(color: Colors.grey, fontSize: 12)))
                else
                  Column(
                    children: inventoryItems
                        .map((item) =>
                            _buildMobilePartCard(item, isFromTask: true))
                        .toList(),
                  )
            ],
          ),
  ...
  Widget _buildRelatedParts() {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) =>
          _buildMobilePartCard(filteredParts[index]),
    );
  }
  ...
  Widget _buildMobilePartCard(dynamic item, {bool isFromTask = false}) {
    ...
    int currentQty = isSelected
        ? _selectedPartsMap[uniqueKey]['quantity']
        : (item['quantity'] ?? 0);
    ...
    return Container(
      // Убрано const для исправления ошибки компиляции
      margin: EdgeInsets.only(
          bottom: 8, left: isFromTask ? 8 : 0, right: isFromTask ? 8 : 0),
      padding: const EdgeInsets.all(12),
      decoration: BoxDecoration(
        color: isFromTask ? Colors.grey.shade50 : Colors.white,
        borderRadius: BorderRadius.circular(8),
        border: isFromTask ? null : Border.all(color: Colors.grey.shade200),
        boxShadow: isFromTask
            ? null
            : [
                BoxShadow(
                ...
                Row(
                  children: [
                    if (article.isNotEmpty)
                      _badge(
                          article, Colors.blue.shade50, Colors.blue.shade700),
                    const SizedBox(width: 8),
                    if (!isFromTask)
                      Text("На складе: ${item['quantity']}",
                          style: const TextStyle(
